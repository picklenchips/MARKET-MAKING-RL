...
class PolicyGradient(object):
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            T = len(rewards)
            returns = [0]*T  # initialize G_t as T-long list
            returns[T-1] = rewards[T-1]  # set G_T to the last reward
            for i in range(T-1):
                idx = T - i - 2  # index
    ...
    def normalize_advantage(self, advantages):
        ...
        # can potentially divide by zero if std is zero?
        normalized_advantages = (advantages - np.mean(advantages))/(np.std(advantages)+1e-10)
        #######################################################
        #########          END YOUR CODE.          ############
